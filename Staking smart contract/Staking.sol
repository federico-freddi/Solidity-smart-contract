...
contract Staking {
    ...
    function addToken(
     ...
     external onlyOwner {
        ...
        tokenAddress,
        usdPrice,
        usdPrice/ ethUsdPrice, //in this way we have the eth price.
        apy
        );
    ...
    function stakeTokens (string calldata symbol, uint tokenQuantity) external {
        require(tokens[symbol].tokenId != 0, "You can't stake this token");

        //We use the IERC20 library
    ...
    function getPositionByIds(uint positionId) external view returns(Position memory){
    ...
    function closePosition(uint positionId) external {
        ...
        require(positions[positionId].open == true, "this position is already close");
    ...
    function modifyCreatedDate(uint positionId, uint newCretatedDate) external onlyOwner{
        positions[positionId].createDate = newCretatedDate;
    }
} 